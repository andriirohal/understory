...
export function refreshAccountSummary(): void {
...
function resetAccountStats(): void {
  const orders = document.querySelector<HTMLElement>("[data-account-orders]");
  ...
  if (plants) {
    plants.textContent = "0";
  }

  if (loyalty) {
    loyalty.textContent = getLoyaltyLevel(0);
  }

  if (lastOrder) {
    lastOrder.textContent = t("header.accountPopover.noOrders");
  }
}
...
function loadAccountSummary(userId: string): void {
  if (summaryUserId === userId && accountSummary) {
    updateAccountStats(accountSummary);

    return;
  }

  if (summaryUserId === userId && summaryPromise) {
    return;
  }
...
export function initAccountUI(): void {
  ...
  window.addEventListener("order-changed", refreshAccountSummary);
}
