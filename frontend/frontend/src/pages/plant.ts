...
import { t } from "../i18n";
import { addToCart, getAllPlants, getCart } from "../api";
...
async function getFreshPlant(plantId: string): Promise<Plant | null> {
  try {
    const result = await getAllPlants(100, 0);

    const plants = Array.isArray(result.data)
      ? result.data
      : Array.isArray(result)
        ? result
        : [];

    return plants.find((plant: Plant) => plant.id === plantId) ?? null;
  } catch (error) {
    console.error("Failed to get fresh plant:", error);
...
function updateQuantityButtons(quantityControl: HTMLElement): void {
  ...
  const addButton = document.querySelector<HTMLButtonElement>(SELECTORS.add);
...
export async function renderPlant(
  ...
  const freshPlant = await getFreshPlant(id);
        ...
        <div class="plant_info">
          ...
          <div class="purchase_row">
            ...
            <div
              ...
              aria-label="${t("plant.quantity")}"
            >

              <button
                type="button"
                class="qty_btn qty_minus"
                aria-label="${t("plant.decreaseQuantity")}"
                disabled
              >
              ...
              <button
                ...
                aria-label="${t("plant.increaseQuantity")}"
            ...
            <button
              ...
              id="add_to_cart_btn"
              data-id="${plant.id}"
              ${currentPlantStock <= 0 || availableQuantity <= 0 ? "disabled" : ""}
            >
              ${t("plant.addToBag")}
            </button>
